...
async def run_pipeline(video_id: str, video_source: str, styles: List[str] = None) -> VideoResult:
    ...
    try:
        ...
        # Send all frames to Kimi 2.6 VLM to generate the draft, then verify using Gemma 4 VLM cascade.
        desc = ""
        inference_type = "scenes"
        if frames_extracted:
            start_time = time.time()
            try:
                # 1. Draft generation using Kimi 2.6 VLM
                kimi_client = get_vlm_client(provider="fireworks")
                images = [f["bytes"] for f in frames_extracted]
                
                # Construct chronological frame timing manifest
                manifest_lines = []
                ...
                logger.info("CascadeVLM: Generating draft description using Kimi 2.6 VLM")
                draft_desc = await kimi_client.generate_draft_sequence(images, VLM_SEQUENCE_PROMPT, manifest_text=manifest_text)
                
                # 2. Verification using Gemma 4 VLM Cascade
                from llm.prompts import VLM_VERIFY_PROMPT
                verify_prompt = VLM_VERIFY_PROMPT.format(draft=draft_desc)
                
                logger.info("CascadeVLM: Verification using Gemma 4 VLM Cascade")
                gemma_client = get_vlm_client(provider="gemma4")
                try:
                    raw_desc = await gemma_client.describe_frames_sequence(images, verify_prompt)
                    logger.info("CascadeVLM: Gemma 4 VLM Cascade verification completed successfully.")
                except Exception as gemma_exc:
                    logger.warning(f"CascadeVLM: Gemma 4 VLM Cascade failed: {gemma_exc}. Falling back to Kimi 2.6 verification.")
                    # Fallback verification call on Kimi 2.6
                    raw_desc = await kimi_client.describe_frames_sequence(images, verify_prompt)
                ...
                try:
                    ...
                    vlm_client = get_vlm_client(provider="fireworks")
                    images = [f["bytes"] for f in frames_extracted]
