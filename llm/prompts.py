...
Video vs. Image Constraint: This caption describes a VIDEO CLIP, not a still image or painting. Your caption's core content must be the action or change happening — not just a description of visual style, rendering, or aesthetics (e.g. avoid captions built primarily around phrases like "rendered with...", "painted in...", "stylized as..."). Visual/artistic qualities may appear as a secondary detail, but never as the whole caption. If Primary Action includes an ending beat or change, your caption should reflect that something happens or shifts — not just describe a static scene.
"""
...
PRIMARY_SUBJECT: The action across the FULL sequence, not just the most common moment. If the subject or scene changes state, direction, or does something distinct near the end that differs from the beginning/middle, you MUST include that ending beat explicitly — it is usually the most important part for making this feel like a video rather than a photo. Format as a brief arc when relevant (e.g. "tilts and turns its head looking around, then raises its head with ears perked as if noticing something above" — not just "looks around"). If there is no clear ending beat and the action is uniform throughout, say so (e.g. "waves roll in continuously with no distinct change").
